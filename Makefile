...
# Credentials (from environment variables or make arguments)
# Environment variables are automatically inherited from shell
GITHUB_USERNAME ?= $(shell echo $$GITHUB_USERNAME)
GITHUB_PAT ?= $(shell echo $$GITHUB_PAT)

# Find oras executable (user can override with ORAS_PATH)
ORAS_PATH ?= $(shell command -v oras 2>/dev/null || echo "$$HOME/bin/oras")
ORAS := $(ORAS_PATH)

# Derived variables
DOCKER_REPO := $(REGISTRY)/$(ORG)
HELM_CHART_REPO := $(REGISTRY)/$(ORG)/$(HELM_REPO)
...
debug:
	...
	@if [ -z "$(GITHUB_USERNAME)" ]; then \
		...
		echo "    Run: source .env"; \
		echo "    Or:  export GITHUB_USERNAME=your-username"; \
	fi
	@if [ -z "$(GITHUB_PAT)" ]; then \
		echo "  - GITHUB_PAT not set!"; \
		echo "    Run: source .env"; \
		echo "    Or:  export GITHUB_PAT=ghp_xxxx"; \
	fi
	@if [ ! -x "$(ORAS)" ]; then \
...
help:
	@echo "$(GREEN)Microcron-CE Build & Publish Makefile$(NC)"
	@echo ""
	@echo "$(YELLOW)Usage:$(NC)"
	@echo "  make publish GITHUB_USERNAME=<username> GITHUB_PAT=<token>"
	@echo ""
	@echo "$(YELLOW)Available targets:$(NC)"
	@echo "  $(GREEN)debug$(NC)             - Check if credentials are loaded"
...
helm-publish: helm-package
	...
	cd helm && $(ORAS) push $(OCI_ARTIFACT) $(HELM_CHART):application/vnd.cncf.helm.chart.v1.tar+gzip
...
publish: versions
	...
	@echo "$(GREEN)========================================$(NC)"
	@echo "$(GREEN)Microcron-CE Build & Publish Pipeline$(NC)"
	@echo "$(GREEN)========================================$(NC)"
	@echo ""
...
clean:
	...
	@echo "$(GREEN)✓ Cleaned: helm/$(HELM_CHART)$(NC)"
	@echo "$(YELLOW)Note: Docker images must be removed manually:$(NC)"
